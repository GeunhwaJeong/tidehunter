...
use crate::{
    index_table::IndexTable, key_shape::KeySpaceDesc, lookup::RandomRead, math::rescale_u32,
};
...
impl PersistedIndex for SingleHopIndex {
    fn to_bytes(_table: &IndexTable, _ks: &KeySpaceDesc) -> Bytes {
        todo!()
    }

    fn from_bytes(_ks: &KeySpaceDesc, _b: Bytes) -> IndexTable {
        todo!()
    }

    fn lookup_unloaded(
        _ks: &KeySpaceDesc,
        _reader: &impl RandomRead,
        _key: &[u8],
    ) -> Option<WalPosition> {
        todo!()
    }
